...
exports.enrollAdmin = async (caClient, wallet, orgMspId) => {
	try {
		...
		if (identity) {
			logger.error('An identity for the admin user already exists in the wallet');
			return;
		}
...
exports.registerAndEnrollUser = async (caClient, wallet, orgMspId, userId, affiliation) => {
	try {
		...
		if (userIdentity) {
			logger.error(`An identity for the user ${userId} already exists in the wallet`);
			return;
		}
		...
		if (!adminIdentity) {
			...
			logger.error('Enroll the admin user before retrying');
			return;
		}
