...
function clearContainers() {
  ...
  if [ -z "$CONTAINER_IDS" -o "$CONTAINER_IDS" == " " ]; then
    infoln "1 ===> No containers available for deletion"
  else
...
function removeUnwantedImages() {
  ...
  if [ -z "$DOCKER_IMAGE_IDS" -o "$DOCKER_IMAGE_IDS" == " " ]; then
    infoln "2 ===> No images available for deletion"
  else
...
function checkPrereqs() {
  ...
  infoln "3 ===> LOCAL_VERSION=$LOCAL_VERSION"
  infoln "4 ===> DOCKER_IMAGE_VERSION=$DOCKER_IMAGE_VERSION"
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    infoln "5  ===> $LOCAL_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "1 fatal ===> Local Fabric binary version of $LOCAL_VERSION does not match the versions supported by the test network."
    fi

    infoln "6  ===> $DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "2 fatal ===> Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the test network."
    fi
  done
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "7 ===> CA_LOCAL_VERSION=$CA_LOCAL_VERSION"
    infoln "8 ===> CA_DOCKER_IMAGE_VERSION=$CA_DOCKER_IMAGE_VERSION"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    which cryptogen
    if [ "$?" -ne 0 ]; then
      fatalln "3 fatal ===> cryptogen tool not found. exiting"
    fi
    infoln "9 ===> Generating certificates using cryptogen tool"

    infoln "10 ===> Creating PatientOrg Identities"
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "4 fatal ===> Failed to generate certificates..."
    fi

    infoln "11 ===> Creating HospitalOrg Identities"
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "5 fatal ===> Failed to generate certificates..."
    fi

    infoln "11.1 ===> Creating PharmacyOrg Identities"
    ...
    infoln "12 ===> Creating Orderer Org Identities"
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "6 fatal ===> Failed to generate certificates..."
    fi
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    infoln "13 ===> Generating certificates using Fabric CA"
  ...
  while :
    ...
    infoln "14 ===> Creating PatientOrg Identities"

    createPatientOrg

    infoln "15 ===> Creating HospitalOrg Identities"

    createHospitalOrg

    infoln "15.1 ===> Creating PharmacyOrg Identities"

    createPharmacyOrg

    infoln "16 ===> Creating Orderer Org Identities"

    createOrderer

  fi

  infoln "17 ===> Generating CCP files for PatientOrg, HospitalOrg and PharmacyOrg"
...
function createConsortium() {
  which configtxgen
  if [ "$?" -ne 0 ]; then
    fatalln "7 fatal ===> configtxgen tool not found."
  fi

  infoln "18 ===> Generating Orderer Genesis block"
  ...
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
    fatalln "8 fatal ===> Failed to generate orderer genesis block..."
  fi
}
...
function networkUp() {
  ...
  docker ps -a
  if [ $? -ne 0 ]; then
    fatalln "9 fatal ===> Unable to start network"
  fi
}
...
function deployCC() {
  ...
  if [ $? -ne 0 ]; then
    fatalln "10 fatal ===> Deploying chaincode failed"
  fi
}
...
CLI_DELAY=3
# channel name defaults to "health-channel"
...
if [ "$MODE" == "up" ]; then
  infoln "20 ===> Starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}' ${CRYPTO_MODE}"
elif [ "$MODE" == "createChannel" ]; then
  infoln "21 ===> Creating channel '${CHANNEL_NAME}'."
  infoln "22 ===> If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
elif [ "$MODE" == "down" ]; then
  infoln "23 ===> Stopping network"
elif [ "$MODE" == "restart" ]; then
  infoln "24 ===> Restarting network"
elif [ "$MODE" == "deployCC" ]; then
  infoln "25 ===> deploying chaincode on channel '${CHANNEL_NAME}'"
else
  printHelp
  exit 1
fi
